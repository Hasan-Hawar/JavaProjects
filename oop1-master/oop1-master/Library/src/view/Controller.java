...
public class Controller {
	...
	public void searchAction() throws NoItemsFoundException {
		...
		if (!category.get().isEmpty() && !title.get().isEmpty()) {
			// Todo
			BookItem item = null ;
			if (categories.equals("Buch")) {
				Book book = item.getBook();
				if (item.getDataObject().getTitle().equals(title)) {
					res = Utils.addNumber(res, item.getId());
				}
			}
			if (res == null) {
				throw new NoItemsFoundException("Item Not Found");
			}
		}
	...
	public void checkPassword() throws InvalidLoginException {
		...
		for (Customer customer : customers) {
			if (!(customer.getEmail().equals(email.getValue())) && !(customer.getEmail().equals(password.getValue()))) {
				message.setValue("Invalid User name or Password!");

				throw new InvalidLoginException("Invalid Password");
			} else {
				message.setValue("Password is correct");
			}
			password.setValue("");
		}
	}
	...
	public void nextScreen() throws InvalidLoginException {
		main.clearScreen();
